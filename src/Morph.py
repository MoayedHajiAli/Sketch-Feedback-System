...
class Morph():
    ...
    def start(self):
        ...
        for i in range(n+1):
            ...
            tem_x, tem_y = self.match(self.original[self.original_labels == i], self.target[self.target_labels == i], steps = 200)
        ...
        anim = animation.FuncAnimation(self.fig, func =self.animate,
                                       init_func = self.init_animation, frames = 200, interval = 20, blit = True, repeat_delay=2000, repeat= False)
