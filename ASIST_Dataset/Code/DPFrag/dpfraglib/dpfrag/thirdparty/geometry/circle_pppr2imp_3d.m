...
%  Parameters:
%
%    Input, real P1(3), P2(3), two points on the circle.
%
%    Input, real P3(3), a third point.
%
%    Input, real R, the radius of the circle.
%
%    Output, real PC(3,2), the centers of the two circles.
%
%    Output, real NORMAL(3), the normal to the circles.
%
  dim_num = 3;
%
...
%
  if ( 2.0 * r < dist )
    for j = 1 : 2
      pc(1:dim_num,j) = 0.5 * ( p1(1:dim_num) + p2(1:dim_num) );
    end
    return
  end
%
...
%
  ...
  dot = v(1:dim_num) * ( p2(1:dim_num) - p1(1:dim_num) )';
  dot = dot / dist;

  v(1:dim_num) = v(1:dim_num) - dot * ( p2(1:dim_num) - p1(1:dim_num) ) / dist;

  v_length = sqrt ( sum ( v(1:dim_num).^2 ) );

  v(1:dim_num) = v(1:dim_num) / v_length;
%
%  We can go with or against the given normal direction.
%
  pc(1:dim_num,1) = 0.5 * ( p2(1:dim_num) + p1(1:dim_num) ) + h * v(1:dim_num);

  pc(1:dim_num,2) = 0.5 * ( p2(1:dim_num) + p1(1:dim_num) ) - h * v(1:dim_num);
