...
%
  if ( r8_modp ( theta  - theta1,  2.0 * pi ) <= ...
    ...
    r2 = sqrt ( sum ( ( p(1:dim_num) - center(1:dim_num) ).^2 ) );

    pn(1:dim_num) = center(1:dim_num) + ( p(1:dim_num) - center(1:dim_num) ) * r / r2;
%
...
%
  elseif ( r8_modp ( theta - 0.5 * ( theta1 + theta2 ), 2.0 * pi ) <= pi )
    ...
    pn(1:dim_num) = center(1:dim_num) + r * [ cos ( theta2 ), sin ( theta2 ) ];
%
%  Otherwise, the endpoint associated with THETA1 is closest.
  else

    pn(1:dim_num) = center(1:dim_num) + r * [ cos ( theta1 ), sin ( theta1 ) ];
